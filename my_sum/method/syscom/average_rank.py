...
class AverageRankSummarizer(object):
    LIMIT = float('inf')
    ...
    def __call__(self, documentSet, words_limit, summarizer1, summarizer2, summarizer3):
        return self.average_rank(documentSet, words_limit, summarizer1, summarizer2, summarizer3)

    def average_rank(self, document, words_limit, summarizer1, summarizer2, summarizer3):
        ...
        for idx, (rating1, rating2, rating3) in enumerate(zip(rank1, rank2, rank3)):
            rank[idx] = (rating1 + rating2 + rating3) / 3
        ...
        return summary_sent
        # return ' '.join(sent._texts for sent in summary_sent)
